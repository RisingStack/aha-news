...
function countChildren(commentTree) {
  if (!commentTree) return 0;

  let count = 0;
  for (const branch of commentTree) {
    count += countChildren(branch);
  }

  return count + commentTree.length;
}
...
function createCommentInsertData(commentTree, createdUsers, postId) {
  return {
    ...
    childrenCount: countChildren(commentTree),
  };
}
